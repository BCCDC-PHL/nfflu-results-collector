...
def test_run_summary_matches_golden_output(analysis_dir, tmp_path):
    """The run_summary.csv column set, order, and values are the contract
    downstream ingestion reads. This test pins that contract byte-for-byte against a checked-in golden file, so a change
